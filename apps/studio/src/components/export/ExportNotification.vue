...
export default {
  ...
  data() {
    return {
      ...
      notification: new Noty({
        text: `导出 '${this.exporter.table.name}'`,
        layout: "bottomRight",
        timeout: false,
        closeWith: 'button',
        buttons: [
          Noty.button("取消", "btn btn-flat", this.cancelExport.bind(this)),
          Noty.button("隐藏", "btn btn-primary", () => this.notification.close()),
        ],
        queue: "export",
      }),
    }
  ...
  computed: {
    notificationText() {
      return `(${this.percentComplete}%) 导出 '${this.exporter.table.name}'`
    },
  },
  methods: {
    cancelExport() {
