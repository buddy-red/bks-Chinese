...
<script lang="ts">
  ...
  export default Vue.extend({
    ...
    computed: {
      ...
      contextOptions() {
        return [
          { name: "关闭", slug: 'close', handler: ({item}) => this.close(item)},
          { name: "关闭其它", slug: 'close-others', handler: ({item}) => this.closeOther(item)},
          { name: '全部关闭', slug: 'close-all', handler: this.closeAll},
          { name: "复制", slug: 'duplicate', handler: ({item}) => this.duplicate(item)}
        ]
      },
      lastTab() {
    ...
    methods: {
      ...
      async loadTableCreate(table) {
        ...
        if (!method) {
          this.$noty.error(`找不到脚本 ${table.name} (${table.entityType})`)
          return
        }
      ...
      duplicate(other: OpenTab) {
        ...
        if(tab.type === 'query') {
          tab.title = "查询 #" + (this.tabItems.length + 1)
          tab.unsavedChanges = true
        }
        this.addTab(tab)
      },
